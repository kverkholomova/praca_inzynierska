...
// register with email and password refugee
  Future registerWithEmailAndPasswordRef(String email, String password, String user_name, String phone_number) async {
    try {
      ...
      await DatabaseService(uid: user!.uid).updateUserData('New refugee','2', user_name, phone_number,[], user.uid, 0);
      return _userFromCredUser(user);
    } catch (error) {
      print(error.toString());
      return null;
...
// register with email and password volunteer
  Future registerWithEmailAndPasswordVol(String email, String password) async {
    try {
      ...
      User? user = result.user;
      // create a new document for the user with the uid
      await DatabaseService(uid: user!.uid).updateUserData('New volunteer','1', userName, phoneNumber, chosenCategoryList, user.uid, volunteerRate);
      return _userFromCredUser(user);
    } catch (error) {
      print(error.toString());
      return null;
